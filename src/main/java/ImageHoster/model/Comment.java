...
public class Comment {
    ...
    //One comment can belong to  only one user  but one user can have multiple comments
    //FetchType is LAZY
    @ManyToOne(fetch = FetchType.LAZY)
    @JoinColumn(name = "user_id")
    private User user;

    public Comment(Integer id, String text, Date createdDate, Image image, User user){
        this.id = id;
        this.text = text;
